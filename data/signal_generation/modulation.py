...
def pulse_shape_bits_fir(
    bits: np.ndarray, fir_taps: np.ndarray, sps: int
) -> np.ndarray:
...
def modulate_frequency(
    symbols: np.ndarray, fsk_deviation: float, fs: float
) -> np.ndarray:
...
def oqpsk_modulate(
    ...
    hss_I_chips = pulse_shape_bits_fir(
        np.concatenate((I_chips, [0])), fir_taps=fir_taps, sps=sps
    )
    hss_Q_chips = pulse_shape_bits_fir(
        np.concatenate(([0], Q_chips)), fir_taps=fir_taps, sps=sps
    )
    ...
    hss_Q_chips = np.pad(
        hss_Q_chips, (0, len(hss_I_chips) - len(hss_Q_chips)), mode="constant"
    )
    ...
    iq_signal = iq_signal[
        : len(I_chips) * sps + int(np.ceil(sps / 2)) + 1
    ]  # Magic expression found by inspection
