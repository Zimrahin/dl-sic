...
from .demodulation import single_pole_iir_filter
from .packet import (
...
class ReceiverBLE(Receiver):
    ...
    def demodulate(
        ...
        if demodulation_type == "INSTANTANEOUS_FREQUENCY":
            ...
            # Generate Gaussian taps and convolve with rectangular window
            iq_samples = scipy.signal.correlate(
                iq_samples, self._gauss_taps, mode="full"
            )
            ...
            complex_exp = np.exp(
                ...
                * np.pi
                * self._fsk_deviation
                * np.arange(len(self._gauss_taps))
                / self._sample_rate
            )
            ...
            iq_samples_lower = scipy.signal.correlate(
                iq_samples, gauss_bandpass_lower, mode="full"
            )
            iq_samples_higher = scipy.signal.correlate(
                iq_samples, gauss_bandpass_higher, mode="full"
            )
            ...
            before_symbol_sync = np.real(
                iq_samples_higher_square - iq_samples_lower_square
            )
    ...
    def process_phy_packet(
        ...
        preamble_positions: np.ndarray = correlate_access_code(
            bit_samples,
            generate_access_code_ble(base_address),
            threshold=preamble_threshold,
        )
        ...
        for preamble in preamble_positions:
            ...
            payload_and_crc = pack_bits_to_uint8(
                bit_samples[payload_start : payload_start + total_bytes * 8]
            )
            payload_and_crc, _ = ble_whitening(payload_and_crc, lsfr)

            # CRC check
            header_and_payload = np.concatenate(
                (header, payload_and_crc[: -self._crc_size])
            )
            ...
            crc_check = (
                True
                if (computed_crc == payload_and_crc[-self._crc_size :]).all()
                else False
            )
            ...
            detected_packets.append(
                {
                    ...
                    "length": len(payload),
                    "crc_check": crc_check,
                    "position_in_array": payload_start,
                }
            )
...
class Receiver802154(Receiver):
    ...
    def demodulate(
        ...
        if demodulation_type == "INSTANTANEOUS_FREQUENCY":
            ...
            freq_samples -= single_pole_iir_filter(freq_samples, alpha=160e-6)
            before_symbol_sync = scipy.signal.correlate(
                freq_samples, self.rect_taps, mode="full"
            )
            ...
            # Band-pass filter (complex signal, complex filter, complex output)
            iq_samples_lower = scipy.signal.correlate(
                iq_samples, rect_bandpass_lower, mode="full"
            )
            iq_samples_higher = scipy.signal.correlate(
                iq_samples, rect_bandpass_higher, mode="full"
            )
    ...
    def process_phy_packet(
        ...
        preamble_positions: np.ndarray = preamble_detection_802154(
            chip_samples, preamble_threshold, self.chip_mapping
        )
        detected_packets: list[dict] = []
        ...
        for preamble in preamble_positions:
            ...
            payload_length = pack_chips_to_bytes(
                chip_samples[preamble:payload_start],
                num_bytes=1,
            ...
            if (
                payload_length > self._max_packet_len
            ...
            if CRC_included:
                ...
                crc_check = (
                    True
                    if (computed_crc == payload[-self._crc_size :]).all()
                    else False
                )
